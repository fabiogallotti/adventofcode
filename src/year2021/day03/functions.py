def part_1(data):
    ...
    gamma_rate_binary = [
        "0" if bit_freq["0"] > bit_freq["1"] else "1"
        for bit_freq in bit_columns.values()
    ]
    gamma_rate_dec = int("".join(gamma_rate_binary), 2)

    epsilon_rate_binary = [
        "1" if bit_freq["0"] > bit_freq["1"] else "0"
        for bit_freq in bit_columns.values()
    ]
...
def _calculate_oxygen_rating(data):
    ...
    for col_num in range(len(possible_values[0])):
        ...
        possible_values[:] = [
            elem for elem in possible_values if elem[col_num] == most_common
        ]
...
def _calculate_co2_rating(data):
    ...
    for col_num in range(len(possible_values[0])):
        ...
        possible_values[:] = [
            elem for elem in possible_values if elem[col_num] == least_common
        ]
