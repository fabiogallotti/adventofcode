...
def test_calculate_lights_on():
    assert (
        calculate_lights_on([["turn", "on", "0,0", "through", "999,999"]])
        == 1000000
    )
    assert (
        calculate_lights_on([["turn", "on", "0,0", "through", "999,999"], ["turn", "off", "499,499", "through", "500,500"]])
        == 999996
    )
...
def test_calculate_brightness():
    ...
    assert (
        calculate_brightness([["toggle", "0,0", "through", "999,999"]])
        == 2000000
    )
